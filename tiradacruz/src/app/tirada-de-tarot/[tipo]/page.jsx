...
export async function generateMetadata({ params }) {
  ...
  const title = `Tarot de ${tip.nombre} | Arcanos Online Gratis con IA`
  const description = `${tip.descripcion_seo} Tirada de tarot en cruz con arcanos mayores o baraja completa. Interpretación por IA al instante. Gratis y sin registro. TiradaCruz Argentina.`

  return {
    title,
    description,
    keywords: [...tip.palabras_clave, 'tarot gratis', 'tirada de tarot online', 'arcanos mayores', 'tarot argentina'],
    alternates: { canonical: `${SITE_URL}/tirada-de-tarot/${tipo}` },
    openGraph: {
      title,
      description: tip.urgencia,
      url: `${SITE_URL}/tirada-de-tarot/${tipo}`,
      type: 'website',
      locale: 'es_AR',
      siteName: 'TiradaCruz',
    },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-red-50">
      <header className="text-center py-8 px-4 border-b border-amber-200/50">
        ...
        <p className="text-sm text-amber-700 mb-2 font-medium">🔮 Tarot</p>
      ...
      <main className="max-w-6xl mx-auto px-4 pb-16">
        ...
        <section className="mt-14 max-w-3xl mx-auto">
          <h2 className="text-2xl font-semibold text-amber-900 mb-4">
            Consultá el tarot sobre el {tip.nombre.toLowerCase()}
          </h2>
          <p className="text-amber-800 mb-4">{tip.urgencia}</p>
          <p className="text-amber-800">
            El tarot en cruz de cinco arcanos ilumina tu situación de {tip.nombre.toLowerCase()} desde
            cinco ángulos: lo que está pasando ahora, lo que viene del pasado, lo que viene en el futuro,
            el consejo de los arcanos y el resultado más probable. La IA integra el simbolismo arquetípico
            de cada carta con el contexto de tu pregunta.
          </p>
        </section>
        ...
        <section className="mt-10 max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {ciudadesSample.map((c) => (
              <a
                key={c.id}
                href={`/tirada-de-tarot/${tipo}/${c.id}`}
